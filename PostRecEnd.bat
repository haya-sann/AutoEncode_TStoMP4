...
set "jpg_path=C:\Users\haya_\Desktop\TVjpg"

rem 一時的にmp4を吐き出すフォルダのパス
set "tmp_folder_path=E:\tmp"
rem backup and sync用フォルダのパス
set "bas_folder_path=E:\backupandsync"
rem エンコしたファイルが10GBより大きい、処理に25回失敗した場合にts、ts.program.txt、mp4を退避するフォルダのパス
set "err_folder_path=C:\Users\haya_\Desktop"

REM rem tweet.rbのパス
REM set "tweet_rb_path=C:\DTV\EDCB\tweet.rb"
REM rem SSL証明書のパス
REM set "ssl_cert_file=C:\DTV\EDCB\cacert.pem"
...
echo "%AddKey%" | findstr "Jazz ジャズ 音楽 マイルス Miles Music"
if %errorlevel% equ 0 (
    rem 出力フォルダ作成
...
rem ツイートで警告
REM set "tweet_content=ERROR:%FileName%.tsと関連ファイルを退避しました。ログを確認して下さい。"
REM ruby "%tweet_rb_path%"
rem 異常終了
exit